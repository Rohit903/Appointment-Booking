...
<html>
	...
	<body style="background: url(img1.jpg) no-repeat; width: 100%; height: 150px; background-size: 100%;">
		<center>
			<h1 style = color:brown><b>Booking page<b></h1>
		...
		<!--	<label><b>Email:</b><br></br></label>
			...
			<!--	<input class = "btn-default" type = "submit" value = "Go Back to Login_Pa"\>	-->
			</form>
			<h1>
				<?php
					//$name="";
				...
				/*	if(isset($_POST['address']))
					...
					$result =mysqli_query($link,"select $Day as Availability from Time$name where $Day = 1 and Slots = '$Slot'");
					...
					if($row2['ConsultantName']==$name)
					{
						...
						{	
							...
							else
							{
								...
								echo "slot booked on\t".$Dat."\tbooked slot is\t".$Slot."\ton\t".$Day;	
								mysqli_query($link,"update Time$name set $Day = \"booked\" where  Slots = '$Slot'");
								$temp = 1;
							}
					//	}else{
				 ...
				 */
					...
					{
						echo "consultantname not exists";
					}
					mysqli_close($link);
					}
				/*	if($Book)
